...
test("the production build contains the app shell and migration features", async () => {
  ...
  assert.match(javascript, /Exportar PDF/);
  assert.match(javascript, /Como está na tela/);
  assert.match(javascript, /Grego e transliterado/);
  assert.match(javascript, /Incluir cores/);
  assert.match(javascript, /Incluir sublinhados/);
  assert.match(javascript, /Continuar para impressão/);
  assert.match(pdfDialogSource, /textMode: "screen"/);
  assert.match(pdfDialogSource, /includeColours: true/);
  assert.match(pdfDialogSource, /includeMelismas: true/);
  assert.match(pdfDialogSource, /agrupadas por hino/);
  assert.match(source, /printSettings\.textMode !== "screen"/);
  assert.match(source, /printSettings\.textMode === "both"/);
  assert.match(source, /segments=\{transliteratedSegments\}/);
  assert.match(source, /label=\{printSettings\.textMode === "both" \? "Grego" : undefined\}/);
  assert.match(source, /label=\{printSettings\.textMode === "both" \? "Transliteração" : undefined\}/);
  assert.match(source, /Ao exportar o PDF, você pode escolher o grego, a transliteração ou as duas leituras/);
  assert.doesNotMatch(source, /O PDF continua em grego/);
  ...
  assert.match(stylesSource, /@media print \{[\s\S]*\.print-hide-melismas \.melisma-simple/);
  assert.match(stylesSource, /@media print \{[\s\S]*\.print-presentations \{ display:block; \}/);
